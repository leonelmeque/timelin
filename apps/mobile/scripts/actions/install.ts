...
manager.stdout.on('error', (data) => {
  console.error(`Failed to install ${packageName}`.bgRed);
  console.error(`Error: ${data}`.bgRed);
});

console.log(`Starting to install ${packageName}...`.bgGreen);
...
manager.stdout.on('close', () => {
  console.log(`Finished executing install ${packageName} .\n`.bgCyan);
});
